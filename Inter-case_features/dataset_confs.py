...
logs_dir = "inter_labeled_logs_csv_processed"
...
for formula in range(1,3):
    ...
    filename[dataset] = os.path.join(logs_dir, "inter_traffic_fines_%s.csv"%formula)
    ...
    dynamic_num_cols[dataset] = ["expense", "timesincelastevent", "timesincecasestart", "timesincemidnight", "event_nr", "month", "weekday", "hour", "open_cases",
                                 'nr_cases', 'nr_past_events', 'nr_ongoing_cases', 'cr_cases_intensity','complete_cases_intensity',]
...
for dataset in datasets:
    ...
    filename[dataset] = os.path.join(logs_dir, "inter_%s.csv" % dataset)
...
filename[dataset] = os.path.join(logs_dir, "inter_Production.csv")
...
bpic2017_dict = {"bpic2017_cancelled": "inter_BPIC17_O_Cancelled.csv",
                 "bpic2017_accepted": "inter_BPIC17_O_Accepted.csv",
                 "bpic2017_refused": "interBPIC17_O_Refused.csv"
                }
...
for i in range(1, 7):
    for suffix in ["", "_sample10000", "_sample30000"]:
        ...
        filename[dataset] = os.path.join(logs_dir, "inter_hospital_billing_%s%s.csv" % (i, suffix))
        ...
        dynamic_num_cols[dataset] = ['msgCount', "timesincelastevent", "timesincecasestart", "timesincemidnight", "event_nr", "month", "weekday", "hour",
                                     'nr_cases', 'nr_past_events', 'nr_ongoing_cases', 'cr_cases_intensity','complete_cases_intensity',]#]#, "open_cases"]
...
#### BPIC2012 settings ####
bpic2012_dict = {"bpic2012_cancelled": "inter_bpic2012_O_CANCELLED-COMPLETE.csv",
                 "bpic2012_accepted": "inter_bpic2012_O_ACCEPTED-COMPLETE.csv",
                 "bpic2012_declined": "inter_bpic2012_O_DECLINED-COMPLETE.csv"
                }
...
for municipality in range(1,6):
    for formula in range(1,3):
        ...
        filename[dataset] = os.path.join(logs_dir, "inter_BPIC15_%s_f%s.csv"%(municipality, formula))
...
for formula in range(1,5):
    ...
    filename[dataset] = os.path.join(logs_dir, "inter_BPIC11_f%s.csv"%formula)
...
filename[dataset] = os.path.join(logs_dir, "inter_github_data.csv")
