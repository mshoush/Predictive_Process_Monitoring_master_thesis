"""
This an extended version of code done by "Irene", moreover you can find the original code at below link:

https://github.com/irhete/predictive-monitoring-thesis

This script used to train and evaluate a predictive model to measure the execution time.

"tp run this script:"
                    python experiments_performance.py <data set> <bucketing_encoding> <classifier> <nr_iteration>
                    python experiments_performance.py <data set> <prefix_index_nrEvents> <classifier> <nr_iteration>
                Ex:
                    python experiments_performance.py production single_laststate  catboost 10
                    python experiments_performance.py production prefix_index_10  catboost 10
"Author:"
    ...
    for ii in range(n_iter):
        ...
        for bucket in set(bucket_assignments_test):
            ...
            if cls_method == 'catboost' and bucket_method!='cluster':
                ...
                pd.DataFrame(dt_train_bucket_encoded).to_csv(dataset_name + cls_method + "dt_train_bucket_encoded.csv",
                                                             sep=";",
                                                             index=False)
                del dt_train_bucket_encoded
                pd.DataFrame(dt_test_bucket_encoded).to_csv(dataset_name + cls_method + "dt_test_bucket_encoded.csv",
                                                            sep=";",
                                                            index=False)
                del dt_test_bucket_encoded

                dt_train_bucket_encoded = pd.read_csv(dataset_name + cls_method + "dt_train_bucket_encoded.csv",sep=";")
                os.remove(dataset_name + cls_method + "dt_train_bucket_encoded.csv")

                dt_test_bucket_encoded = pd.read_csv(dataset_name + cls_method + "dt_test_bucket_encoded.csv", sep=";")
                os.remove(dataset_name + cls_method + "dt_test_bucket_encoded.csv")

                for col in list(dt_train_bucket_encoded.select_dtypes(include=['object', 'category']).columns):
                    dt_train_bucket_encoded[col] = dt_train_bucket_encoded[col].astype(str)
                    dt_test_bucket_encoded[col] = dt_test_bucket_encoded[col].astype(str)
                cls.fit(dt_train_bucket_encoded, train_y,
                        list(dt_train_bucket_encoded.select_dtypes(include=['object', 'category','bool']).columns))
            else:
